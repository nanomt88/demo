...
public class HttpClient {
	 ...
	 * @throws Exception
	 */
	public int sendJsonString(String jsonData, String encoding) throws Exception {
		try {
			...
			LogUtil.writeLog("请求报文:[" + jsonData + "]");
			this.requestServer(httpURLConnection, jsonData, encoding);
	 ...
	 * @throws IOException
	 */
	private String response(final HttpURLConnection connection, String encoding) throws URISyntaxException,
		...
		if (null != requestParam && 0 != requestParam.size()) {
			for (Entry<String, String> en : requestParam.entrySet()) {
				try {
					sf.append(en.getKey()
							+ "="
							+ (null == en.getValue() || "".equals(en.getValue()) ? "" : URLEncoder.encode(
							en.getValue(), coder)) + "&");
